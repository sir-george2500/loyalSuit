...
public class DeliveryAgentController {
    ...
    @PreAuthorize("hasRole('DELIVERY_AGENT')")
    @Operation(summary = "Complete one of my deliveries with proof of delivery")
    public ResponseEntity<ApiResponse<DeliveryResponse>> complete(
            @PathVariable UUID id,
            @AuthenticationPrincipal UserPrincipal principal,
            @Valid @RequestBody CompleteDeliveryRequest body) {
        return ResponseEntity.ok(ApiResponse.ok(
                deliveryService.completeAsAgent(id, principal.getTenantId(), principal.getUserId(), body)));
    }
}
