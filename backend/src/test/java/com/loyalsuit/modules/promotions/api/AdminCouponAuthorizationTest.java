...
/**
 * Coupons set prices, so they're owner-only (SUPER_ADMIN / TENANT_ADMIN). The service is
 * mocked — this asserts the authorization wall.
 */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class AdminCouponAuthorizationTest {
    ...
    void listCoupons(UserRole role) throws Exception {
        mvc.perform(get("/api/v1/admin/coupons").header("Authorization", bearer(role)))
                .andExpect(allowedFor(OWNERS, role));
    }

    @ParameterizedTest
    @EnumSource(UserRole.class)
    void createCoupon(UserRole role) throws Exception {
        String body = "{\"code\":\"SAVE10\",\"discountType\":\"PERCENT\",\"value\":10.00}";
    ...
    @ParameterizedTest
    @EnumSource(UserRole.class)
    void updateCoupon(UserRole role) throws Exception {
        String body = "{\"code\":\"SAVE10\",\"discountType\":\"PERCENT\",\"value\":15.00}";
        mvc.perform(put("/api/v1/admin/coupons/" + UUID.randomUUID())
                        .header("Authorization", bearer(role))
                        .contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(allowedFor(OWNERS, role));
    }
    ...
    void anonymousIsRejected() throws Exception {
        mvc.perform(get("/api/v1/admin/coupons")).andExpect(status().is4xxClientError());
    }
}
